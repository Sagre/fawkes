...
;---------------------------------------------------------------------------
;  init.clp - CLIPS active-diagnosis
;
...
(deftemplate active-diagnosis-init-request
...
(deftemplate active-diagnosis-init-signal
	(slot id (type SYMBOL))
...
(defrule active-diagnosis-enable-debug
  (declare (salience ?*SALIENCE-INIT*))
  (active-diagnosis-init)
  (confval (path "/clips-active-diagnosis/debug/enable") (type BOOL) (value TRUE))
  ...
  ;(dribble-on "trace.txt")
	(do-for-fact ((?c confval)) (and (eq ?c:path "/clips-active-diagnosis/debug/level") (eq ?c:type UINT))
		(printout debug "Setting debug level to " ?c:value " (was " ?*DEBUG* ")" crlf)
		(debug-set-level ?c:value)
	)

	(do-for-fact ((?c confval)) (and (eq ?c:path "/clips-active-diagnosis/debug/unwatch-facts")
																	 (eq ?c:type STRING) ?c:is-list)
	 (cx-debug-unwatch-facts ?c:list-value)
  )
	(do-for-fact ((?c confval)) (and (eq ?c:path "/clips-active-diagnosis/debug/unwatch-rules")
																	 (eq ?c:type STRING) ?c:is-list)
...
(deffunction cx-init-indexes (?spec ?stage)
	(bind ?rv (create$))
	(do-for-all-facts ((?c confval)) (str-prefix (str-cat "/clips-active-diagnosis/specs/" ?spec "/init/" ?stage "/")
																							 ?c:path)
...
(deffunction cx-assert-init-requests (?spec ?stage ?feature-default)
	...
	(bind ?cfgpfx (str-cat "/clips-active-diagnosis/specs/" ?spec "/init/" ?cfg-stage "/"))
	(foreach ?i (cx-init-indexes ?spec ?cfg-stage)
		...
		(assert (active-diagnosis-init-request (state ?state) (error-msgs ?error-msgs)
...
(defrule active-diagnosis-init-start
  ...
  (confval (path "/clips-active-diagnosis/spec") (type STRING) (value ?spec))
	...
	(assert (active-diagnosis-init-stage STAGE-1))
)

(defrule active-diagnosis-init-failed
  (declare (salience ?*SALIENCE-INIT*))
	(active-diagnosis-init)
	(active-diagnosis-init-request (state ERROR) (stage ?stage) (order ?i) (error-msgs $?error-msgs))
	?sf <- (active-diagnosis-init-stage ?stage)
	=>
	(printout error crlf)
	(printout error "***********************************************************" crlf)
	(printout error crlf)
	...
	(printout error crlf)
	(retract ?sf)
	(assert (active-diagnosis-init-stage FAILED))
)

(defrule active-diagnosis-init-stage-request-feature
	(active-diagnosis-init)
	(active-diagnosis-init-stage ?stage)
...
(defrule active-diagnosis-init-stage-request-no-feature
	(active-diagnosis-init)
	(active-diagnosis-init-stage ?stage)
	?ir <- (active-diagnosis-init-request (state PENDING) (stage ?stage) (order ?order)
...
(defrule active-diagnosis-init-stage-request-feature-unavailable
	(active-diagnosis-init)
	(active-diagnosis-init-stage ?stage)
	?ir <- (active-diagnosis-init-request (state PENDING) (stage ?stage) (order ?order)
																 (name ?name) (feature TRUE))
...
(defrule active-diagnosis-init-stage-request-feature-fulfilled
	(active-diagnosis-init)
	(active-diagnosis-init-stage ?stage)
	?ir <- (active-diagnosis-init-request (state FEATURE-REQUESTED) (stage ?stage) (name ?name))
...
(defrule active-diagnosis-init-stage-request-files
	(active-diagnosis-init)
	(active-diagnosis-init-stage ?stage)
	?ir <- (active-diagnosis-init-request (state FEATURE-DONE) (stage ?stage) (name ?name)
																 (files $?files) (wait-for ?wait-for))
	=>
	(if (> (length$ ?files) 0)
	 then
...
(defrule active-diagnosis-init-stage-wait-for-ok
	(active-diagnosis-init)
	(active-diagnosis-init-stage ?stage)
	?ir <- (active-diagnosis-init-request (state WAIT-FOR) (stage ?stage) (name ?name)
																 (wait-for ?wait-for))
...
(defrule active-diagnosis-init-stage-wait-for-fail
	(active-diagnosis-init)
	(active-diagnosis-init-stage ?stage)
	?ir <- (active-diagnosis-init-request (state WAIT-FOR) (stage ?stage) (name ?name)
																 (wait-for ?wait-for))
	?ws <- (active-diagnosis-init-signal (id ?wait-for) (ok FALSE) (error-msg ?error-msg))
	=>
	(retract ?ws)
	(modify ?ir (state ERROR) (error-msgs ?error-msg))
)

(defrule active-diagnosis-init-stage-finished
	(active-diagnosis-init)
	?sf <- (active-diagnosis-init-stage ?stage)
	(not (active-diagnosis-init-request (state ~COMPLETED) (stage ?stage)))
	=>
	(retract ?sf)
